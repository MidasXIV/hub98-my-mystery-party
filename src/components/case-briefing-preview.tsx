...
const PreviewStyles = () => (
  <style jsx global>{`
    ...
    .manila-envelope {
      ...
      box-shadow: 0 2px 5px rgba(0,0,0,0.2), inset 0 0 20px rgba(0,0,0,0.05);
    }

    /* 2. The Flap Shadow (Triangular look) */
    .envelope-flap {
      background: linear-gradient(135deg, rgba(0,0,0,0.05) 0%, transparent 40%);
      pointer-events: none;
    }
...
export default function CaseBriefingPreview({ content }: { content: string }) {
  ...
  return (
    <div className="w-full h-full p-2 relative group cursor-pointer select-none">
      ...
      {/* --- Envelope Container --- */}
      <div className="w-full h-full manila-envelope flex flex-col relative overflow-hidden rounded-sm transition-transform duration-300 hover:scale-[1.02]">
        ...
        <div className="absolute top-[25%] right-4 z-20 flex flex-col items-center gap-4">
           ...
           <div className="absolute top-[6px] left-[5px] w-4 h-10 pointer-events-none string-wrap">
              <svg width="20" height="40" viewBox="0 0 20 40" fill="none">
                 ...
                 <path d="M0 0 C 10 5, 10 15, 0 20 C -10 25, 10 35, 0 40" stroke="#d84315" strokeWidth="1.5" />
              </svg>
           </div>
           {/* Bottom Button */}
           <div className="closure-button" />
        ...
        <div className="flex-1 flex flex-col justify-end p-3 z-10 mt-8">
           ...
           {/* "To" Block */}
           <div className="mb-2">
             <span className="font-mono text-[8px] uppercase text-gray-600 block mb-0.5">Delivery To:</span>
